...
description: >
  ...
  "readability check", "thin content", "content audit", "content score",
  "improve my content", "content gaps", "is my content good enough",
  "content review", "helpful content", or "content optimization".
---
...
**CRITICAL:** Different CMS platforms render visible text in different HTML patterns.
You MUST extract ALL visible content patterns, not just one. Always verify word
counts by spot-checking at least one page's full extracted text against the rendered
page to ensure nothing is missed.

**Duda sites** use multiple `<span>` style patterns for visible text:
- `<span style="display: unset">` — headings, nav labels, taglines, short UI text
- `<span style="display: initial">` — body paragraphs, FAQ answers, descriptive content
- **You MUST grep for BOTH patterns** or you will massively undercount content.
- Use: `grep -o '<span[^>]*display: \(unset\|initial\)[^>]*>[^<]*</span>'`
- **Remove shared header/footer boilerplate LINES, not a flat word count.** Extract
  text from 3+ different page types (e.g. service page, about, contact, gallery),
  find lines common to ALL using `comm -12` or `grep -vxF`, then remove those
  exact lines from each page's output before counting words. Do NOT estimate a
  flat word deduction — page-specific headings and taglines vary and must not be
  stripped. Typical Duda boilerplate lines: nav links, footer service category
  links, membership badges, trading hours, ABN.

**WordPress** — extract from `<div class="entry-content">` or `<article>` elements.

**Shopify** — extract from `<div class="rte">` or product description containers.

**Other/unknown platforms** — strip all HTML tags, then manually exclude obvious
nav/footer/sidebar repeated text. Always cross-check one page visually.
...
> 3-pillar SEO report. Findings feed into Content Quality & E-E-A-T
> (15% of overall Health Score).
