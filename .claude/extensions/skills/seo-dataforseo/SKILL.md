...
description: >
  ...
  "search volume", "keyword difficulty", "keyword research", "backlink data",
  "competitor data", "AI visibility check", "LLM mentions", "real search data",
  "check my rankings", "what keywords does [domain] rank for", "competitor
  keyword gap", "how much traffic does [site] get", "keyword ideas",
  "search intent", "keyword trends", "who links to [site]", "domain authority",
  "spam score", "Lighthouse audit", "technology stack", "WHOIS",
  "ChatGPT visibility", "LLM brand mentions", or "live keyword data".
  Always use this skill when real-time data is needed rather than estimates
  — it provides the live numbers that make audits and strategies actionable.
---
...
These workflows sequence multiple DataForSEO tools to answer common SEO questions efficiently.

### Workflow 1: Full Competitor Teardown
*"What's working for [competitor] and why?"*

```
1. serp_organic_live_advanced        → What are they ranking #1-3 for?
2. dataforseo_labs_google_ranked_keywords → Full ranked keyword list (top 100)
3. dataforseo_labs_bulk_traffic_estimation → Estimated organic traffic
4. backlinks_summary                 → Domain authority and link profile overview
5. backlinks_anchors                 → What anchor text patterns do they use?
6. dataforseo_labs_google_competitors_domain → Who else are they competing against?
```

### Workflow 2: Keyword Gap Analysis
*"What keywords should I be targeting that I'm not?"*

```
1. dataforseo_labs_google_ranked_keywords (your domain) → Your current rankings
2. dataforseo_labs_google_ranked_keywords (competitor)  → Their rankings
3. dataforseo_labs_google_domain_intersection           → Keywords you share
   → Subtract to find: keywords THEY rank for that YOU don't
4. dataforseo_labs_bulk_keyword_difficulty              → Score gap keywords
5. dataforseo_labs_search_intent                        → Classify intent
   → Prioritise: high volume + low difficulty + commercial/transactional intent
```

### Workflow 3: New Site Keyword Research
*"What should I write about for [topic/niche]?"*

```
1. dataforseo_labs_google_keyword_ideas (seed keyword) → Broad topic map
2. dataforseo_labs_google_related_keywords             → Semantic expansion
3. kw_data_google_ads_search_volume                    → Volume confirmation
4. dataforseo_labs_bulk_keyword_difficulty             → Competition scoring
5. dataforseo_labs_search_intent                       → Intent classification
   → Output: prioritised keyword clusters by intent + difficulty
```

### Workflow 4: Backlink Profile Health Check
*"Is my link profile safe? Do I have toxic links?"*

```
1. backlinks_summary                      → Overview: DR, total links, domains
2. backlinks_bulk_spam_score              → Identify high spam-score domains
3. backlinks_anchors                      → Check for over-optimised anchor text
4. backlinks_timeseries_summary           → Check for unnatural velocity spikes
5. backlinks_bulk_new_lost_backlinks      → Recent gains and losses
   → Flag: spam score >45, exact-match anchors >10%, sudden spike
```

### Workflow 5: GEO / AI Visibility Check
*"Is [brand] being cited by AI search engines?"*

```
1. ai_optimization_chat_gpt_scraper       → What does ChatGPT say about [query]?
   → Which sources is it citing? Is [brand] among them?
2. ai_opt_llm_ment_search                 → Find LLM mentions of [brand/keyword]
3. ai_opt_llm_ment_top_domains            → Which domains dominate AI citations?
4. ai_opt_llm_ment_cross_agg_metrics      → Compare across ChatGPT vs Claude vs Perplexity
   → Identify: are competitors being cited more? What content are they citing?
```

### Workflow 6: SERP Opportunity Analysis
*"Is there a winnable ranking opportunity for [keyword]?"*

```
1. serp_organic_live_advanced             → Who's ranking top 10?
2. backlinks_bulk_ranks (top 10 domains)  → Domain authority of current rankers
3. dataforseo_labs_bulk_keyword_difficulty → Difficulty score
4. dataforseo_labs_search_intent          → Confirm intent matches your page
5. kw_data_google_ads_search_volume       → Monthly search volume
   → Score: volume ÷ difficulty × intent match = opportunity score
```
